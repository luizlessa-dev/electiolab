...
3. Mapear campos reais das amostras contra as specs. Registrar em `docs/arquitetura.md` (seção "Mapa de campos"): chaves de totalização (`and`, `snt`, `st`, `pst`…; `nv` = vagas do cargo, `vag` = vagas por agrupamento no proporcional), estrutura de candidatos, votos válidos/brancos/nulos, IDG.
...
## Fase 1 — Schema
1. Revisar `apuracao-2026/migrations/0001_apuracao.sql` (rascunho) à luz do mapa de campos. Ajustar, incluindo FK **anulável** de `votacao_candidato` para `public.candidates` via `tse_id` (o casamento `sqcand`↔`tse_id` fica para a Fase 2, contra o oficial; ver `docs/arquitetura.md` → "Consequências para a Fase 1").
...
4. `src/lib/apuracao/resultados.ts`: baixa só os EA20 que mudaram. Escopo do 1º turno — **todos os cargos em disputa na abrangência UF**:
   ...
   - Governador (27 UFs)
   - Senador (27 UFs; 2 vagas em 2026 — número de vagas vem de `carg[].nv`, não de `vag`)
   - **Deputado Federal (27 UFs)**
   - **Deputado Estadual (26 UFs) e Deputado Distrital (DF)**
   - ~~Conselheiro Distrital (eleição 6261)~~ — **fora do escopo** (decisão 26/09/2026: é o Conselho Distrital de Fernando de Noronha, não do DF).
...
2. Checklist de virada para `TSE_AMBIENTE=oficial` (códigos 6257/6259 (federal/estadual) conferidos contra `ele-c.json` oficial; 6261 fora do escopo).
