...
export type CandidateViewProps = {
  ...
  c: CandidateDetail;
  slug: string;
  /** Path canônico desta renderização, usado no JSON-LD. */
  canonicalPath: string;
  /** Todas as eleições da mesma pessoa, para o seletor. */
  elections: CandidateElectionOption[];
};

/**
 * O corpo da página de candidato, compartilhado por /candidato/<slug> e
 * /candidato/<slug>/<segmento>. As duas rotas rendem a mesma coisa; o que muda
 * é qual linha de `candidates` (isto é, qual eleição) chega em `c`.
 */
export async function CandidateView({ c, slug, canonicalPath, elections }: CandidateViewProps) {
  ...
  const jsonLd = {
    ...
    "@graph": [
      {
        "@type": "Person",
        "@id": `https://electiolab.com${canonicalPath}#person`,
        name: c.name,
        description: c.bio,
        ...
        url: `https://electiolab.com${canonicalPath}`,
        ...
        <section className="grid md:grid-cols-[1fr_280px] gap-8">
          <div className="flex gap-5">
            ...
            <p className="text-xs font-mono text-muted-foreground uppercase tracking-wider mb-2">
              Pesquisas Eleitorais {election?.year ?? 2026}
            </p>

            {/*
              Seletor de eleição. Uma pessoa costuma ter mais de uma linha em
              `candidates` — um turno é uma eleição separada nesta base, e quem
              disputa dois cargos no mesmo ciclo também tem duas. Sem isto, só a
              eleição que vence o desempate de getCandidateBySlug tinha página:
              o 2º turno de 2022 (Lula, Bolsonaro) ficava inalcançável.
            */}
            {elections.length > 1 && (
              <nav
                aria-label={`Outras eleições de ${c.name}`}
                className="flex flex-wrap items-center gap-1.5 mb-4"
              >
                {elections.map((o) => {
                  const atual = o.candidateId === c.id;
                  const href = o.isPrimary
                    ? `/candidato/${slug}`
                    : `/candidato/${slug}/${o.segment}`;
                  const classe =
                  ...
                  return atual ? (
                    <span
                      key={o.segment}
                      aria-current="page"
                      className={`${classe} border-primary bg-primary/10 text-foreground font-medium`}
                    >
                      {o.election.name}
                    </span>
                  ) : (
                    <Link
                      key={o.segment}
                      href={href}
                      className={`${classe} border-border text-muted-foreground hover:border-primary/50 hover:text-foreground`}
                    >
                      {o.election.name}
                    </Link>
                  );
