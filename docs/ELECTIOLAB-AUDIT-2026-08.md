...
**Atualização 2026-08-19:** C1, C2, C3, C5 e C6 resolvidos; C4 parcialmente (a parte mecânica — `changeFrequency` e a copy que prometia cadência semanal — está corrigida; a decisão de fundo sobre retomar ou não a série editorial `/relatorio/semana-N` segue em aberto, não bloqueante). Ver os próprios achados abaixo pra detalhe de commit. Os scores da seção 1 são o snapshot original de 13/08 e ainda não foram recalculados — a nota real hoje é bem mais alta que a tabela mostra, especialmente em Produto (C1), SEO técnico (C2/C3) e GEO (C6).
...
### ✅ C3 — RESOLVIDO em 2026-08-17 — Cache ISR não está funcionando em `candidato/[slug]`, `instituto/[slug]`, `partido/[slug]` (todas as 3 famílias de rota dinâmica)
...
- **✅ RESOLVIDO em 2026-08-17, duas causas reais, nenhuma delas Vercel:** (1) `queries.ts` usava o client de cookies pra ler dado 100% público — trocado pelo client anônimo (commit `3e909df`); (2) causa principal, valia pras 3 rotas: **nenhuma tinha `generateStaticParams`**, e a doc oficial do Next.js é explícita — sem retornar um array (mesmo vazio), "the route will be dynamically rendered", `revalidate` sozinho não basta pra registrar a rota no pipeline de ISR da Vercel (commit `735e638`). Confirmei via API da Vercel que Fluid Compute estava ligado no projeto — pista falsa, não era a causa. Testado ao vivo pós-deploy: as 3 rotas retornam `x-vercel-cache: HIT` + `x-nextjs-prerender: 1` + `age` crescente na 2ª request.

### 🟡 C4 — PARCIALMENTE RESOLVIDO — Relatórios semanais parados há ~11 semanas, num produto que se anuncia como "atualizado semanalmente"
...
- **✅ Ação imediata resolvida em 2026-08-19** (commit `f0b37b9`): `changeFrequency` já estava `never` desde o PR #58. FAQ de `/sobre` reescrita pra não prometer "semanalmente... automaticamente" — descreve o processo real (sync diário PesqEle + curadoria por fonte primária). `/imprensa` linkava "Relatório semanal" pra semana-17 (a mais antiga) — trocado pra semana-22 (mais recente), rótulo sem "semanal".
- **Ação estrutural (decisão de fundo) já endereçada por outro caminho**: em vez de retomar os relatórios editoriais manuais, a ingestão diária de pesquisas via PesqEle (achado em [[project_electiolab_pesqele_wikipedia_agente2]]) virou o mecanismo real de "produto sempre atualizado" — ver pipeline de curadoria em `scripts/pending-polls.ts`. Decisão sobre retomar ou não a série `/relatorio/semana-N` como conteúdo editorial em si continua em aberto, não é bloqueante.

### ✅ C5 — RESOLVIDO em 2026-08-19 — Person duplicado/conflitante e uso indevido de `AggregateRating` no schema da página mais replicada do site
...
- **✅ RESOLVIDO em 2026-08-19** (commit `f0b37b9`): `Person` duplicado removido de `candidate-schema.tsx` (o `CreativeWork` referencia via `@id` cross-script o `Person` que `page.tsx` já declara, com dado mais completo). `AggregateRating` trocado por `additionalProperty`/`PropertyValue`, mesmo padrão de `instituto/[slug]`.
